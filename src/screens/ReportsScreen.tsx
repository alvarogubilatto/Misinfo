...
function DonutChart({ period }: DonutChartProps) {
    const { slices } = reportData[period] as { slices: any[] }
    ...
    const paths = slices.map((item, i) => {
        ...
        sa = ea
        return <path key={i} d={path} fill={item.c} style={{ animation: `screenIn 0.4s ${i * 0.08}s both` } as React.CSSProperties} />
    })
    return (
...
export default function ReportsScreen({ className, showToast, showSuccess }: ReportsScreenProps) {
    const [period, setPeriod] = useState('2025')
    const [dlBtnText, setDlBtnText] = useState('Descargar Paquete Fiscal 2025')
    const data = reportData[period] as { total: string, slices: any[] }

    const downloadAll = () => {
        setDlBtnText('Preparando paquete...')
        setTimeout(() => {
            setDlBtnText('Descargar Paquete Fiscal 2025')
        ...
        <div className={className}>
            <div className="reports-inner">
                ...
                <div className="info-box">
                    <div style={{ width: 18, height: 18, flexShrink: 0, marginTop: 1, color: 'var(--primary)' }}><Icon name="tool" /></div>
                ...
                <div>
                    ...
                    <div className="docs-card">
                        <div className="doc-item" onClick={() => showToast('Descargando: Constancia de Intereses Reales')}>
                            <div className="doc-icon-wrap"><Icon name="news" /></div>
                            <div>
                                <div className="doc-name">Constancia de Intereses Reales</div>
                                <div className="doc-date">Generado hoy</div>
                            </div>
                            <div className="doc-chip"><Icon name="download" /></div>
                        </div>
                        <div className="doc-item" onClick={() => showToast('Descargando: Comprobantes Inmobiliarios 2025')}>
                            <div className="doc-icon-wrap"><Icon name="news" /></div>
                            <div>
                                <div className="doc-name">Comprobantes Inmobiliarios 2025</div>
                                <div className="doc-date">Generado hace 1 mes</div>
                            </div>
                            <div className="doc-chip"><Icon name="download" /></div>
                        </div>
                        <div className="doc-item" onClick={() => showToast('Seleccionar archivos para subir')}>
                            <div className="doc-icon-wrap"><Icon name="tool" /></div>
                            <div>
                                <div className="doc-name">Facturas de Mantenimiento</div>
                                <div className="doc-date" style={{ color: 'var(--orange)' }}>
                                    <Icon name="warning" style={{ width: 12, height: 12, display: 'inline-block', marginRight: 4 }} />
                                    Faltan 2 facturas — Toca para subir
                                </div>
                            </div>
                            <div className="doc-chip" style={{ color: 'var(--orange)' }}><Icon name="tool" /></div>
                        </div>
                    </div>
                </div>

                <button className="btn-download-all" onClick={downloadAll}>
                    <div style={{ width: 20, height: 20, marginRight: 10, display: 'inline-block', verticalAlign: 'middle' }}>
                        <Icon name={dlBtnText.includes('...') ? 'loading' : 'download'} className={dlBtnText.includes('...') ? 'spin' : ''} />
                    </div>
                    {dlBtnText}
                </button>
                <div style={{ height: 28 }} />
